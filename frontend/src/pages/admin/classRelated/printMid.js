...
const divisionMid = getDivisionMid(totalEndGrade);
 ...
 // Function to get the class name based on the ID
 const getClassName = (classID) => {
  if (sclassDetails) {
    // Check if sclassDetails is an array or an object and find the class name
    const foundClass = Array.isArray(sclassDetails) 
      ? sclassDetails.find((sclass) => sclass._id === classID)
      : sclassDetails[classID]; // Assuming sclassDetails is an object with IDs as keys
    
    return foundClass ? foundClass.sclassName : ''; // Return class name or empty string
  }
  return ''; // Return empty string if sclassDetails is not available
};

const className = getClassName(filteredStudent.sclassName);
      ...
      >
        ...
        <Box display="flex" justifyContent="space-between">
          ...
          <Box sx={{ flex: 1, borderRight: '1px solid black' }}>{totalGrade}</Box>
                    <Box sx={{ flex: 1 }} style={{ textTransform: 'uppercase', color: 'red', }}>{division}</Box>
        </Box>
        </Box>
        {/* MID TERM  */}
        <Box sx={{ flex: 2, borderRight: '1px solid black' }}>
        ...
        <Box display="flex" justifyContent="space-between">
          ...
          <Box sx={{ flex: 1 }} style={{ textTransform: 'uppercase', color: 'red', }}>{divisionMid}</Box>
        </Box>
